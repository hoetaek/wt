...
mod tests {
    ...
    fn substitutes_single_variable() {
        let result = render(
            ...
            &vars(&[("site_name", "sample-app-proj-680")]),
        );
        assert_eq!(result, "https://sample-app-proj-680.test");
    }
    ...
    fn substitutes_multiple_variables() {
        let result = render(
            "{{repo}}-{{issue_slug}}",
            &vars(&[("repo", "sample-app"), ("issue_slug", "proj-680")]),
        );
        assert_eq!(result, "sample-app-proj-680");
    }
