...
fn try_main() -> Result<()> {
    ...
    if cli.json && !supports_json(command) {
        bail!(
            "JSON output is supported for: wt version, wt list, wt inspect, wt task list, wt workflow list, wt agent status, wt agent watch, wt msg check-inbox, wt doctor, wt profile"
        );
    }
