---
name: writing-tasks
description: "Use when authoring or revising a wt TaskDocument body (execution/tasks/*.toml) before launch — after ready slicing, when a body lacks exact file paths, stepwise tests, commands with expected output, or when a launched agent drifted or stalled because the body under-specified the work."
---
...
- The slice graph (slice titles, dependencies, parallel groups, execution
  ...
  ready handoff report or a leaf workspace. Do not duplicate step detail
  there.
...
separate workflow execution, the prep pass must return to ready after the
bodies are written. ready owns:
...
- reporting the TaskDocument mapping, linked workflow TOML path, `work`
...
1. `## 계획 (Planning)` — slice metadata from ready: 유형 (type), 예상 소요
...
6. **Workflow handoff check** — if the slice graph has dependencies, parallel
   groups, or any non-direct execution shape, verify that ready will create
