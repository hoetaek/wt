...
name: wt-autopilot
description: |
  Use after wt-ready has handed off an approved launch target (launch command,
  ...
  on "$wt-autopilot", "wt autopilot", "launch target approved, run the rest
  automatically", or "after wt-ready, carry it the rest of the way". Do not use
  before the wt-ready handoff, for unclear or unprepared work (route to wt-ready),
...
`wt-autopilot` carries one wt work item after the human-reviewed `wt-ready`
handoff. It does not remove wt's judgment boundary; it moves the boundary to the
wt-ready handoff, then runs the remaining lifecycle — execution, review, landing,
...
and workflow-linked task boundaries. `wt-autopilot` does not decide readiness
gates itself; route unclear or unprepared work through `wt-ready`, which owns
that preparation model.

## Core Contract

- **Handoff first.** Do not start unless `wt-ready` has produced an approved
  ...
  `wt-ready` (which routes thinking to `leaf-work` when needed).
- **Autopilot after the handoff.** Once the launch target is approved, proceed
  automatically: launch and coordinate with `wt-work`, then land and settle with
  `wt-land`. Stop only when a hard stop or pre-authorization gap appears.
- **Review still happens.** Code review, checks, and spec sync are automatic
  ...
  skill keeps it.
- **Do not duplicate the lifecycle skills.** Invoke and follow `wt-ready`,
  `wt-work`, and `wt-land` when their contracts apply. This skill orchestrates
...
3. Confirm the `wt-ready` handoff exists and is approved: launch command, inspect
...
Unprepared or vague work returns to `wt-ready`, not into autopilot.
...
1. **Consume the handoff.** Treat the approved launch target as the current
   ...
   scope), return to `wt-ready`, record why, and do not continue on the old
   contract.
2. **Execute and coordinate with `wt-work`.** Launch the prepared task or
...
4. **Land and settle with `wt-land`.** Respect workflow landing policy, perform
...
- the `wt-ready` handoff is missing, stale, contradicted, or not approved;
...
- Starting before the `wt-ready` handoff is approved.
...
- Rewriting `wt-ready` / `wt-work` / `wt-land` contracts inside this skill.
