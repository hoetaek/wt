...
Use this skill after `work` review says the work is acceptable, after
explicit discard, or when the user asks for a retrospective on closed wt work.
Do not use it to monitor active agents or request fixes; use `work` for that.
...
Use the same semantic coordinator id used during `work`, such as
