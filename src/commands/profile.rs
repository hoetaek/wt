use crate::config::{AgentCli, AgentConfig, Config, IssueProviderType, ReadyMode, SubmitMode};
...
fn list(ctx: &Ctx) -> Result<()> {
    ...
    if profiles.is_empty() {
        ...
        ctx.ui
            .print_step("No profiles found. Create one with: wt profile <name>");
        return Ok(());
    }
...
fn create(ctx: &Ctx, name: &str) -> Result<()> {
    ...
    let base_config = if ctx.config == Config::default() {
        loaded_base_config = Config::load(&ctx.repo_root)?;
        &loaded_base_config
    } else {
        &ctx.config
    };
