...
"#,
    ...
    fn workflow_stack_prompt_uses_draft_pr_handoff_policy() {
        ...
        assert!(!content.contains("wt msg send --scope workflow:"));
        assert!(content.contains(
            "wt workflow pass /repo/.git/wt/execution/workflows/2026-05-16-001.toml PROJ-2 --run-next"
        ));
    }
    ...
    fn workflow_stack_pass_status_messages_quote_shell_args() {
        ...
        let expected_command = "wt workflow pass '/repo/.git/wt/execution/workflows/work flow.toml' 'PROJ weird'\\''s task'";
    ...
    fn workflow_matrix_prompt_uses_scoped_coordinator_handoff() {
        ...
        assert!(!content.contains("wt msg send --scope workflow:"));
        assert!(content.contains(
            "workflow pass /repo/.git/wt/execution/workflows/2026-05-17-002.toml matrix-task:alpha"
        ));
        assert!(!content.contains("--run-next"));
    }
    ...
    fn workflow_matrix_prompt_includes_report_only_coordinator_handoff() {
        ...
        assert!(
            content.contains(
                "wt workflow pass /repo/.git/wt/execution/workflows/test.toml task:alpha"
            )
        );
    }
