...
1. **Human-reviewed handoff** — the `ready` launch target (launch command,
...
- the `land` retrospective for close-out findings, timing, and watch cadence.

## Return Rule

If coordination finds the handoff is wrong (bad slicing, wrong policy, changed
scope or core), return to `ready`. Do not patch around it downstream. Record
