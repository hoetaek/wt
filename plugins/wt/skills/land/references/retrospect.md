...
Use this reference from `land` to capture a closed work item, or a blocked
`autopilot` lesson, as a structured retrospective that future coordination,
...
`land` owns the retrospect step after landing or explicit discard. It can
also record a blocked-loop lesson when the loop stops earlier and the blocker
itself is worth preserving.

## When to Use

- After `land` proves a branch landed and cleanup ran.
- After an intentional discard with explicit user direction.
- At the end of a `autopilot` loop, even when a step blocked progress, if
...
- Closed retrospect: the work item is landed, explicitly discarded, or
  otherwise closed after `land` proved integration/discard and cleanup
  safety. Record outcome, evidence, merge or discard proof, cleanup, and
  lessons.
- Blocked lesson retrospect: the `autopilot` loop stopped before landing —
...
skills = ["autopilot", "ready", ...]   # skills actually used in this loop
...
owner = "<wt | autopilot | ready | work | land | coordinator | <user>>"
...
retrospect files. Keep it small enough for `ready` and `work` to read
...
- Name the owner. Use the same vocabulary as `[[action_candidates]]`
  (`wt | autopilot | ready | work | land | coordinator | <user>`)
...
- SKILL.md bodies under `<wt-repo>/plugins/wt/skills/*/SKILL.md` (installed views are
  symlinks), especially `land/SKILL.md` when the lesson changes closeout
  behavior.
...
1. Classify the retrospect:
   ...
   - Blocked lesson: confirm the `autopilot` loop is stopped at a named
