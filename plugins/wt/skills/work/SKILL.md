---
name: work
description: "Use after ready when prepared wt work should be launched or running wt work should be coordinated: verify readiness, run a task or workflow, capture the inspect target, monitor status, send feedback, review diffs/checks, pass workflow tasks when applicable, and hand accepted work to land."
---
...
| land / merge / cleanup with `wt done` | `land` |
...
- **Adjust in `work`** when the change is a downstream re-scope: reorder,
  ...
  `internal` or `domain`).
- **Return to `ready`** when the discovery changes purpose or requirements
