---
name: welcome
description: "Use to onboard a user to wt by explaining the mental model, first machine setup, the wt skill family, common `wt run` starts, and when to continue into $config or $autopilot without overwhelming them."
---
...
3. **wt skills.** Keep this short:
   - `$config`: inspect the current repo and recommend repo config.
   - `$autopilot`: after `$ready` hands off an approved launch target, run
     the work, review it, land or discard it, and settle reusable lessons into
     the harness.
   - Phase skills `$ready`, `$work`, and `$land` can be run on their own
     for per-step control; `$autopilot` chains `$work` and `$land`
     automatically after the `$ready` handoff.
...
5. **Next action.** End the basic onboarding by telling the user to run
   `$config` for the current repo. If they want actual work executed after
   config is ready, point them to `$autopilot`.
